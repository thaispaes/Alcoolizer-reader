...
public class TableActive {
    ...
    public static List<GSActive> readForDesc(String desc) {
//RawData,RecordNum,MainSerial,MainFWV,SampleSerial,SampleFWV,DateD,TimeT,EventType,DownloadDate,DownloadTime,ResultQualitative,NameFile
        ...
        List<GSActive> produtos = new ArrayList<>();//Array com os dados que vou adicionar sobre o rank 

        try {
            String v = null;
            try{
            v = selecao.getSelectedItem().toString();}catch(NullPointerException e){
            ...
            if (valor == 0) {
                ...
                stm = con.prepareStatement("SELECT * FROM Active WHERE MONTH(DateD)=" + v + ";");
            }
            if (valor == 1) {
                stm = con.prepareStatement("SELECT * FROM Active WHERE YEAR(DateD) =" + selecao.getSelectedItem().toString() + ";");
            }

            if (valor == 3) {
                stm = con.prepareStatement("SELECT * FROM Active WHERE NameFile='" + v + "' ;");
            }
            if (valor == 2) {
                stm = con.prepareStatement("SELECT * FROM Active ORDER BY MainSerial ASC;");
            }
            ...
            while (rs.next()) {
                ...
                GSActive produto = new GSActive();//Array com o metodo gs desta tabela
                //retorno os dados
                ...
                produto.setTrueResult(rs.getString("TrueResult"));
                produto.setNameFile(rs.getString("NameFile"));
                produtos.add(produto);
            }
            stm = con.prepareStatement("SELECT count(*) FROM Active");//Faço uma referencia com o que vou executar futuramente
            ...
            if (rs.next()) {
                ...
                if (va != 0) {
                    //Se for diferente de 0 ele vai entrar aq
...
//Metodo abaixo serve apenas para ordenar a tabela
    public static void readJTableForDesc(String desc) {
        try {
            ...
            modelo.setNumRows(0);
            TableActive.readForDesc(desc).forEach((p) -> {
                modelo.addRow(new Object[]{
                    p.getId(),
                    p.getRawData(),
                    p.getRecordNum(),
